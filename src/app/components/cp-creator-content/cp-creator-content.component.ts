            ...
            if (item.childNodes && item.childNodes.length > 0) {
              ...
              item.innerHTML = find(this.data.content, item.id).content
                .map((text: any) => text?.type === 'form-control' ?
                  text?.formType === 'text' ? `<input type="text" id="${text?.key}" placeholder="${text?.label}" class="form-control text" value="${text?.value}" />` :
                    text?.formType === 'textarea' ? `<textarea  id="${text?.key}" placeholder="${text?.label}" class="form-control textarea">${text?.value}</textarea>` : ''
