...
export class LunarCalendarComponent implements OnInit, AfterViewInit {
    ...
    this.route.params.subscribe((param: any) => {
      ...
      this.selectedDate.solar = new Date(
        `${param.year || this.datePipe.transform(new Date(), 'YYYY')}-${
          ...
          this.calendarMode == 'month'
            ? '01'
            : param.date || this.datePipe.transform(new Date(), 'dd')
        } 00:00:00`
      );
  ...
  getThanhSoInMember() {
    if (this.filter?.yellow) {
      if (!localStorage?.getItem('memberThanhSo')) {
        this.expaned = true;
        this.guestMessage =
          'Bạn có thể chọn Thánh Sở để hiển thị lịch hành đạo và các sự kiện tại Thánh Sở của bạn tại đây. Nếu bạn chưa thấy Thánh Sở của bạn. Hãy Đăng ký thành viên lịch để các sự kiện tại Thánh Sở của bạn sẽ hiển thị trên lịch của CaoDaiON nhé!';
      ...
      this.selectedMonth.forEach((date: any) => {
        if (this.filter?.consecutive) {
          ...
          for (
            ...
            const foundDate = this.currentUser.congPhu?.find(
              (item: any) => item?.dateTime?.toString() == dateValue?.toString()
            );
            const focusArray = foundDate?.data?.map((item: any) => item?.focus);
            const focusSum = focusArray?.reduce((a: any, b: any) => a + b, 0);
            const qualityArray = foundDate?.data?.map(
              (item: any) => item?.quality
            );
            const qualitySum = qualityArray?.reduce(
              (a: any, b: any) => a + b,
              0
            );
          ...
          const foundConsecutive = data?.find((cp: any) => {
            return (
              new Date(cp?.date?.setHours(0))?.toString() ===
              date?.solar?.toString()
            );
          });
          if (
            new Date(
              new Date(
                new Date(date?.solar.setHours(0)).setMinutes(0)
              ).setSeconds(0)
            ) <
            new Date(
              new Date(
                new Date(
                  new Date(new Date().setDate(new Date().getDate())).setHours(0)
                ).setMinutes(0)
              ).setSeconds(0)
            )
          ) {
        ...
        if (this.filter?.blue) {
          ...
          if (foundLunaryearlyMonthlySpecial?.length > 0) {
            if (
              date?.lunar?.convertSolar2Lunar?.lunarDay === 1 ||
              date?.lunar?.convertSolar2Lunar?.lunarDay === 15
            ) {
              foundLunaryearlyMonthlySpecial?.forEach((flms: any) => {
                const soTemplate = flms?.event.key.includes('thoi-ty')
                  ? 'so-soc-vong-thoi-ty'
                  : flms?.event.key.includes('thoi-ngo')
                  ? 'so-soc-vong-thoi-ngo'
                  : '';
                const eventName = flms?.event.key.includes('thoi-ty')
                  ? 'Sớ Sóc Vọng (Thời Tý)'
                  : flms?.event.key.includes('thoi-ngo')
                  ? 'Sớ Sóc Vọng (Thời Ngọ)'
                  : '';
                if (soTemplate) {
        ...
        if (this.filter?.red) {
          ...
          if (foundLunaryearlySpecialSpecial?.length > 0) {
            foundLunaryearlySpecialSpecial.forEach((flyss: any) => {
              ...
              switch (flyss?.event.date) {
                case 'yearly-01-01':
                  soTemplate = flyss?.event.key.includes('thoi-ty')
                    ? 'so-nguon-dan-thoi-ty'
                    : flyss?.event.key.includes('thoi-ngo')
                    ? 'so-nguon-dan-thoi-ty'
                    : '';
                  eventName = flyss?.event.key.includes('thoi-ty')
                    ? 'Sớ Ngươn Đán (Thời Tý)'
                    : flyss?.event.key.includes('thoi-ngo')
                    ? 'Sớ Ngươn Đán (Thời Ngọ)'
                    : '';
                  break;
                case 'yearly-01-09':
                  soTemplate = flyss?.event.key.includes('thoi-ty')
                    ...
                    : '';
                  eventName = flyss?.event.key.includes('thoi-ty')
                    ? 'Sớ Lễ Vía Đức Ngọc-Hoàng Thượng Đế (Thời Tý)'
                    : flyss?.event.key.includes('thoi-ngo')
                    ? 'Sớ Lễ Vía Đức Ngọc-Hoàng Thượng Đế (Thời Ngọ)'
                    : '';
                  break;
                case 'yearly-01-15':
                  soTemplate = flyss?.event.key.includes('thoi-ty')
                    ...
                    : '';
                  eventName = flyss?.event.key.includes('thoi-ty')
                    ? 'Sớ Lễ Thượng Ngươn (Thời Tý)'
                    : flyss?.event.key.includes('thoi-ngo')
                    ? 'Sớ Lễ Thượng Ngươn (Thời Ngọ)'
                    : '';
                  break;
                case 'yearly-02-15':
                  soTemplate = flyss?.event.key.includes('thoi-ty')
                    ...
                    : '';
                  eventName = flyss?.event.key.includes('thoi-ty')
                    ? 'Sớ Lễ Vía Đức Thái Thượng Lão Quân (Thời Tý)'
                    : flyss?.event.key.includes('thoi-ngo')
                    ? 'Sớ Lễ Vía Đức Thái Thượng Lão Quân (Thời Ngọ)'
                    : '';
                  break;
                case 'yearly-02-25':
                  soTemplate = flyss?.event.key.includes('thoi-ty')
                    ...
                    : '';
                  eventName = flyss?.event.key.includes('thoi-ty')
                    ? 'Sớ Lễ Kỷ Niệm Đức Giáo Tông Đắc Đạo (Thời Tý)'
                    : flyss?.event.key.includes('thoi-ngo')
                    ? 'Sớ Lễ Kỷ Niệm Đức Giáo Tông Đắc Đạo (Thời Ngọ)'
                    : '';
                  break;
                case 'yearly-03-13':
                  soTemplate = flyss?.event.key.includes('thoi-ty')
                    ...
                    : '';
                  eventName = flyss?.event.key.includes('thoi-ty')
                    ? 'Sớ Lễ Kỷ Niệm Đức Giáo Tông Thọ Phong (Thời Tý)'
                    : flyss?.event.key.includes('thoi-ngo')
                    ? 'Sớ Lễ Kỷ Niệm Đức Giáo Tông Thọ Phong (Thời NGỌ)'
                    : '';
                  break;
                case 'yearly-04-08':
                  soTemplate = flyss?.event.key.includes('thoi-ty')
                    ...
                    : '';
                  eventName = flyss?.event.key.includes('thoi-ty')
                    ? 'Sớ Lễ Vía Đức Phật Tổ (Thời Tý)'
                    : flyss?.event.key.includes('thoi-ngo')
                    ? 'Sớ Lễ Vía Đức Phật Tổ (Thời Ngọ)'
                    : '';
                  break;
                case 'yearly-05-26':
                  soTemplate = flyss?.event.key.includes('thoi-ty')
                    ...
                    : '';
                  eventName = flyss?.event.key.includes('thoi-ty')
                    ? 'Sớ Lễ Kỷ Niệm Sanh Nhật Đức Giáo Tông (Thời Tý)'
                    : flyss?.event.key.includes('thoi-ngo')
                    ? 'Sớ Lễ Kỷ Niệm Sanh Nhật Đức Giáo Tông (Thời Ngọ)'
                    : '';
                  break;
                case 'yearly-07-15':
                  soTemplate = flyss?.event.key.includes('thoi-ty')
                    ...
                    : '';
                  eventName = flyss?.event.key.includes('thoi-ty')
                    ? 'Sớ Lễ Trung Ngươn (Thời Tý)'
                    : flyss?.event.key.includes('thoi-ngo')
                    ? 'Sớ Lễ Trung Ngươn (Thời Ngọ)'
                    : '';
                  break;
                case 'yearly-08-15':
                  soTemplate = flyss?.event.key.includes('thoi-ty')
                    ...
                    : '';
                  eventName = flyss?.event.key.includes('thoi-ty')
                    ? 'Sớ Lễ Vía Đức Diêu Trì Kim Mẫu (Thời Tý)'
                    : flyss?.event.key.includes('thoi-ngo')
                    ? 'Sớ Lễ Vía Đức Diêu Trì Kim Mẫu (Thời Ngọ) (Cúng các Thánh Thất) '
                    : '';
                  break;
                case 'yearly-10-15':
                  soTemplate = flyss?.event.key.includes('thoi-ty')
                    ...
                    : '';
                  eventName = flyss?.event.key.includes('thoi-ty')
                    ? 'Sớ Lễ Hạ Ngươn (Thời Tý)'
                    : flyss?.event.key.includes('thoi-ngo')
                    ? 'Sớ Lễ Hạ Ngươn (Thời Ngọ)'
                    : '';
                  break;
                default:
                  break;
          ...
          if (foundSolaryearlySpecialSpecial?.length > 0) {
            ...
            foundSolaryearlySpecialSpecial?.forEach((fsyss: any) => {
              const soTemplate = fsyss?.event.key.includes('thoi-ty')
                ? 'so-le-sanh-nhut-duc-gia-to-giao-chu-thoi-ty'
                : fsyss?.event.key.includes('thoi-ngo')
                ? 'so-le-sanh-nhut-duc-gia-to-giao-chu-thoi-ngo'
                : '';
              const eventName = fsyss?.event.key.includes('thoi-ty')
                ? 'Sớ Lễ Sanh Nhựt Đức Gia Tô Giáo Chủ (Thời Tý)'
                : fsyss?.event.key.includes('thoi-ngo')
                ? 'Sớ Lễ Sanh Nhựt Đức Gia Tô Giáo Chủ (Thời Ngọ)'
                : '';
              if (soTemplate) {
      ...
      if (this.calendarMode === 'month') {
        this.selectedMonth.forEach((date: any) => {
          const foundEvent = tuanCuu?.event.find((item: any) => {
            return (
              new Date(item?.solar).getDate() ==
                new Date(date?.solar).getDate() &&
              new Date(item?.solar).getMonth() ==
                new Date(date?.solar).getMonth() &&
              new Date(item?.solar).getFullYear() ==
                new Date(date?.solar).getFullYear()
            );
          });
          if (foundEvent) {
            const foundTitle = CAODAI_TITLE.data.find(
              (cdontt: any) => cdontt.key === tuanCuu.details.title
            );
            foundEvent.eventName = `${foundTitle?.eventTitle} ${
              foundEvent.eventName
            } cho ${foundTitle?.name} ${
              tuanCuu.details.holyName
                ? tuanCuu.details.holyName + ' - ' + tuanCuu.details.name
                : tuanCuu.details.name
            }`;
            if (!date.event || date.event?.length == 0) {
              date.event = [];
            }
            if (
              !date.event?.find((de: any) => de.name == foundEvent.eventName)
            ) {
              date.event.push({
                event: foundEvent,
                details: tuanCuu?.details,
                date: tuanCuu?.date,
              });
            }
          }
  ...
  selectedEvents = <any>[];
  isCtrlHolding: boolean = false;
  openEventSummaryDialog(
    date: any,
    event: any,
    eventSummayDialog: any
  ) {
    const addEvent = () => {
      if (!event?.event?.name) {
      ...
      if (!event?.event?.date) {
        event.event.date = `${event?.event?.day || ''} ngày ${
      ...
      if (!this.selectedEvents?.find((item: any) => item?.event?.event?.key == event?.event?.key)) {
        this.selectedEvents.push({ date, event });
      } else {
        this.selectedEvents[this.selectedEvents.indexOf(this.selectedEvents?.find((item: any) => item?.event?.event?.key == event?.event?.key))] = null
        this.selectedEvents = this.selectedEvents?.filter((item: any) => !!item);
      }
    };
    if (this.isCtrlHolding) {
      addEvent();
    } else {
  ...
  onMouseWheel(event: any) {
    if (
      event?.srcElement?.nodeName !== 'MAT-LIST-ITEM' &&
      event?.srcElement?.nodeName !== 'MAT-LIST' &&
      ...
      if (data.solar) {
        const request = {
          ...
          dates: [
            new Date(data.solar),
            new Date(
              new Date(new Date(data.solar).toJSON()).setHours(
                new Date(data.solar).getHours() + 1
              )
            ),
          ],
          subTitle: '',
        ...
        if (data.dateType === 'lunar') {
          if (
            data?.time[0] === 'ty-2301' ||
            data?.time[0] === 'meo-0507' ||
            data?.time[0] === 'ngo-1113' ||
            data?.time[0] === 'dau-1719'
          ) {
            const startDate = `${
              JSON.parse(JSON.stringify(data.time[0])).split('-')[0]
            }-${JSON.parse(JSON.stringify(data.time[0]))
              .split('-')[1]
              .slice(0, 2)}-${JSON.parse(JSON.stringify(data.time[0]))
              .split('-')[1]
              .slice(-2)}`;
            const request = <any>{
              text: data.name,
            ...
            if (
              data.date.match(new RegExp('yearly-[0-9][0-9]-[0-9][0-9]')) ||
              data.date.match(new RegExp('yearly-monthly-[0-9][0-9]'))
            ) {
              let startDateV = new Date(
                new Date(
                  JSON.parse(JSON.stringify(this.shownDate.date.solar))
                ).setHours(parseInt(data?.time[0].split('-')[1].slice(0, 2)))
              );
              let endDateV = new Date(
                new Date(
                  JSON.parse(JSON.stringify(this.shownDate.date.solar))
                ).setHours(parseInt(data?.time[0].split('-')[1].slice(-2)))
              );
              if (data?.time[0] === 'ty-2301') {
                startDateV = new Date(
                  startDateV.setDate(startDateV.getDate() - 1)
                );
              }
              request.dates = [startDateV, endDateV];
            }
            const url =
              this.calendarService.getGoogleCalendarEventEditUrl(request);
            window.open(url, '_blank');
          }
        }
      }
  ...
  getThanhSoEvent() {
    if (this.selectedThanhSo) {
      this.eventService
        .fetchThanhSoEvent(this.selectedThanhSo)
        .subscribe((res: any) => {
          if (res.status === 200) {
        ...
        if (item.eventType?.includes('ky-niem')) {
          ...
          if (!item?.isSolarEvent) {
            ...
            if (
              new Date() <
              new Date(
                `${newYearTime[2]}-${this.decimal.transform(
                  newYearTime[1],
                  '2.0-0'
                )}-${this.decimal.transform(newYearTime[0], '2.0-0')}`
              )
            ) {
              operationYear -= 1;
            }
            ...
            const convertLunar2Solar =
              this.calendarService.getConvertedFullDate(
                eventLunar
              ).convertLunar2Solar;
            solar = new Date(
              `${convertLunar2Solar[2]}-${this.decimal.transform(
                convertLunar2Solar[1],
                '2.0-0'
              )}-${this.decimal.transform(convertLunar2Solar[0], '2.0-0')}`
            );
            if (item?.thoi) {
              const foundThoi = TIME_TYPE.data.find((tt: any) =>
                tt.name.includes(item?.thoi)
              )?.key;
              const startDate = `${
                JSON.parse(JSON.stringify(foundThoi)).split('-')[0]
              }-${JSON.parse(JSON.stringify(foundThoi))
                .split('-')[1]
                .slice(0, 2)}-${JSON.parse(JSON.stringify(foundThoi))
                .split('-')[1]
                .slice(-2)}`;
            ...
            subject = {
              ...
              details: {
                ...
                job: item?.data?.job,
                color: this.commonService.generatedSlug(
                  item?.data?.color || ''
                ),
  ...
  onOpenSoanSo() {
    ...
    if (
      this.shownDate?.event?.event?.key &&
      typeof this.shownDate?.event?.event?.key === 'string'
    ) {
    ...
    this.generateToken(data).subscribe((tk: any) => {
  ...
  showShareImage() {
    ...
    this.shareBottomSheetRef.afterDismissed().subscribe(() => {
      this.selectedEvents = <any>[]
    });
  }
  ...
  updateShareInformation() {
    this.selectedEvents?.forEach((shareItem: any, index: any) => {
      ...
      shareItem.name = shareItem.event?.event?.name;
      
      if (!this.shareItem.time)
        this.shareItem.time = shareItem?.event?.date;
      if (this.shareItem.time) {
        if (typeof this.shareItem.time == 'string') {
          shareItem.id = this.shareItem.time;
        }
        const selectedEvent = this.shareItem.options?.find(
          (item: any) => item.key == this.shareItem.time
        );
        shareItem.name =
          selectedEvent?.data?.name || shareItem.event?.event?.name;
        shareItem.targetEvent =
          selectedEvent?.data?.name ||
          `${
            this.lunarTimeZone?.find((item: any) =>
              item?.name?.includes(
                shareItem?.date?.event[0]?.event?.eventLunar?.lunarTime
              )
            )?.name
          } (${this.shareItem.time?.time})`;
        if (selectedEvent?.data?.name.includes(shareItem.name)) {
          shareItem.targetEvent = null;
        }
      } else {
        if (shareItem?.event?.mainEventKey) {
          shareItem.event = <any>{};
          shareItem.event.date = 'yearly-';
          shareItem.id = shareItem?.event?.mainEventKey;
          const nameSplit =
            shareItem?.event?.event?.name?.split('|')?.length > 1
              ? shareItem?.event?.event?.name?.split('|')[1]
              : shareItem?.event?.event?.name?.split('thời')[0];
          shareItem.name = nameSplit;
          if (
            shareItem?.event?.mainEventKey?.includes('soc') ||
            shareItem?.event?.mainEventKey?.includes('vong')
          ) {
            if (shareItem.event.event.eventLunar?.lunarMonth) {
              shareItem.name += `tháng ${this.decimal.transform(
                shareItem.event.event.eventLunar?.lunarMonth,
                '2.0-0'
              )}`;
            }
            shareItem.event.date = 'yearly-monthly-';
          }
          this.shareItem.options = shareItem?.date?.event?.map(
            (item: any) => {
              return {
                key: item?.event?.key,
                data: item?.event,
          ...
          if (shareItem?.date?.event) {
            shareItem.targetEvent = this.lunarTimeZone?.find((item: any) =>
              item?.name?.includes(
                shareItem?.date?.event?.find(
                  (de: any) =>
                    de?.event?.solar?.split('T')[0] === shareItem?.date?.d
                )?.event?.eventLunar?.lunarTime
              )
            )?.name;
          }
        }
      }
      if (this.shareItem.targetEvent) {
        shareItem.targetEvent = this.shareItem.targetEvent
      }
      if (!this.shareItem?.events?.find((item: any) => item == shareItem)) {
        this.shareItem.events.push(shareItem);
      }
    });  
    
    this.selectedEvents?.forEach((shareItem: any, index: any) => {
      if (shareItem?.targetEvent == this.selectedEvents[index + 1]?.targetEvent && index != this.selectedEvents?.length - 1) {
        shareItem.targetEvent = null
      };
    })
    this.shareItem.targetEvent = this.selectedEvents[0]?.targetEvent;    
    if (!this.shareItem.id) {
      this.shareItem.id = Date.now();
    }
  }

  isSelectedItem(date: any, event: any) {
    return this.selectedEvents.find((item: any) => item?.date == date && item?.event == event)
  }
      ...
      if (saveItem) {
        html2canvas(saveItem)
          .then((canvas) => {
            ...
            link.download = `${this.commonService.generatedSlug(
              element.id
            )}.png`;
            link.click();
  ...
  shareVegetarianDay(item: any) {
    ...
    this.vegetarianBottomSheetRef = this.matBottomSheet.open(
      this.vegetarianBottomSheet
    );
  }

  addTuanCuu(date: any) {
    this.router.navigateByUrl(
      `/lich/tinh-tuan-cuu/${this.datePipe.transform(date.solar, 'yyyy/MM/dd')}`
    );
  }
  newEvent = <any>{
    data: <any>{},
