...
export class MonthViewComponent implements OnInit, OnDestroy {
  ...
  generateCalendarDays(): void {
    if (this.currentDate) {
      ...
      const lucTrai = [1, 8, 14, 15, 23];
      const thapTrai = [1, 8, 14, 15, 18, 23, 24];
      const thienNguon = [1, 4, 6, 8, 9, 14, 15, 16, 18, 19, 23, 24, 25];
      this.calendarDays.forEach((day: any, index: any) => {
        ...
        if (lucTrai.includes(day.lunar.day) || nextDay?.lunar.day === 1) {
          day['6'] = true;
          day['10'] = true;
          day['16'] = true;
        }
        if (thapTrai.includes(day.lunar.day) || nextTwoDay?.lunar.day === 1) {
          day['10'] = true;
          day['16'] = true;
        }
        if (thienNguon.includes(day.lunar.day) || nextThreeDay?.lunar.day === 1) {
          day['16'] = true;
        }
      });
    }
